    ...
    items: [
      {
        ...
        // Dashboard é acessível para todos os perfis autenticados
      },
      {
        name: 'Relatórios',
        href: '/relatorios',
      ...
      },
      {
        ...
        description: 'Dashboard com gráficos e KPIs',
        permissions: ['reports.read']
      }
    ]
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        permissions: ['payment_requests.read']
      },
      {
        name: 'Pagamentos',
        href: '/pagamentos',
    ...
    items: [
      {
        ...
        // Configurações é acessível para todos os perfis autenticados
      }
    ]
  }
];
...
export function Sidebar({ isOpen, onToggle }: SidebarProps) {
  ...
  const canUserSeeItem = useCallback((item: NavItem) => {
    // Se o item não tem restrições de permissão ou role, todos podem ver
    ...
    // Verificar roles
    if (item.roles) {
      const hasRequiredRole = item.roles.some(role => hasRole(role as Role));
  ...
  const filteredNavSections = useMemo(() => {
    return navSections.map(section => ({
      ...section,
      items: section.items.filter(canUserSeeItem)
  ...
  const filteredSections = useMemo(() => filteredNavSections
    .map(sec => ({
      ...sec,
    ...
    .filter(s => s.items.length > 0), [filter, filteredNavSections]);
